...
@SideEffectFree
@Tags({ "中投国信", "正确数据", "数据质量检测" })
@CapabilityDescription("正确数据处理")
public class ZtgxRightDataHandleProcessor extends AbstractProcessor {
    ...
    public ZtgxRightDataHandleProcessor(){
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        try{
            ...
            JSONObject jo = validateInsertOrUpdate( context, extractData, "ZTGX_QYJBXX_RIGHT");
...
//                jsonObjectNew.put("TYSHXYDM",tyshxydm);
//                jsonObjectNew.put("QYMC",qymc);
//                //查询id是否存在
//                boolean isExit = isExit(id,context,"ZTGX_QYJBXX_RIGHT");
//                if (isExit){
...
//            }
            ...
            getLogger().info("##############################################正确数据updateData："+updateData.size());
            getLogger().info("##############################################正确数据insertData："+insertData.size());
    ...
    private boolean isExit(String id, ProcessContext context, String tablename) {
        ...
        try {
            ...
            while (rs.next()){
                flag = true;
                getLogger().info("..............正确ID："+rs.getString("ID")+"存在");
            }
    ...
    private JSONObject validateInsertOrUpdate(ProcessContext context, JSONArray arr,String tablename){
        Connection conn = null;
        PreparedStatement stmt = null;
        Statement stmts = null;
        ResultSet rs = null;
        ...
        try {
            ...
            for (int j = 0;j<arr.size();j++){
                ...
                String tyshxydm = jsonObject1.getString("TYSHXYDM");
                String qymc =  jsonObject1.getString("QYMC");
                JSONObject jsonObjectNew  = new JSONObject();
                jsonObjectNew.put("ID",id);
                jsonObjectNew.put("TYSHXYDM",tyshxydm);
                jsonObjectNew.put("QYMC",qymc);
